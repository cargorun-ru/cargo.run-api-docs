...
В этом разделе приведены единые форматы данных, используемые во всех методах API CARGO.RUN Logistics 2.0.  
Соблюдение этих правил позволяет избегать ошибок сериализации, неоднозначной интерпретации данных и проблем при интеграции.
...
### Основной формат
Во всех методах API используется единый стандарт:
ISO 8601 (UTC)
...
В нем испльзуется формат без часового пояса: 
YYYY-MM-DDTHH:mm:ss
...
### Причины требования
Единый формат дат облегчает синхронизацию данных между 1С, ERP, мобильным приложением и backend CARGO.RUN.
...
### Причины требования
Обеспечивает однозначность сериализации и корректную работу с внешними базами данных.
