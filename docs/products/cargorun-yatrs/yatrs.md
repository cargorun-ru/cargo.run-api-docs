...
- `lcv` — профиль для малотоннажных машин (до 3.5 тонн).
...
| `Tolls-M12DRT` | Исключать все платные дороги, кроме участка М-12 Дюртюли - Ачит. |
| `TRR` | ТТК. |
| `MRR` | МКАД. |
| `CRR` | ЦКАД. |
...
Помимо сезонных ограничений при этой опции запрещаются все дороги с сомнительным покрытием. Иначе маршрутизатор старается любым способом построить маршрут и использует для этого проселочные дороги.
