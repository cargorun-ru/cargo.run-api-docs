...
Если во внешней системе хранится ID справочника, можно фильтровать элементы по полям, доступным в swagger и настройках конкретного клиента. Для точного значения из `GetListForExternal` достаточно запроса по `id`.
